...
async function checkTest123Data() {
  try {
    console.log('🔍 test123 kullanıcısının verilerini kontrol ediliyor...\n');

    // 1. test123 kullanıcısını bul
    ...
    if (userResult.rows.length === 0) {
      console.log('❌ test123 kullanıcısı bulunamadı!');
      return;
    }

    const user = userResult.rows[0];
    console.log('👤 test123 kullanıcısı bulundu:');
    console.log(user);
    console.log('');
